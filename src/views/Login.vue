    ...
    submit() {
      ...
      if (this.username.trim().length > 0 && this.password.trim().length >= 6) {
        ...
        login(this.username, password, "none", this.deviceId).then((res) => {
          ...
          Cookies.set("wms-admin-Token", res.data.accessToken, {
            expires: expiresIn,
          });
          // Cookies.set("wms-admin-Refresh-Token", res.data.refresh_token, {expires: new Date(new Date().getTime() + 600000 * 1000)});
          Cookies.set("wms-admin-Expires-In", expiresIn, {
            expires: expiresIn,
          });
