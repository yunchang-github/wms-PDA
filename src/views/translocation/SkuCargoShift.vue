<template>
  <div id="SkuCargoShift">
    <van-nav-bar title="Bulk-cargo Shift">
      <template #left>
      ...
      <div class="vanCeliContainer">
        ...
        <van-cell-group>
          <van-field
            ...
            label-width="120px"
            :placeholder="
              disabledBoxNo ? 'Prohibit filling in' : 'Click to type location'
            "
...
export default {
  ...
  methods: {
    // 移位
    async chiftBtn() {
      let currentItem = this.list.find((item) => item.sku === this.sku);
      //还原adjustQuantity
