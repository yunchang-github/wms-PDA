<template>
  <div id="LocationMerge">
    <van-nav-bar title="Location Shift">
      <template #left>
          ...
          >
            <template #button>
              <div style="line-height: 0">
                <van-button size="mini" type="warning" @click="resetDataFun"
                  >Clear</van-button
                >
              </div>
            </template>
  ...
  methods: {
    ...
    async chiftBtn() {
      if (this.list.length === 0)
        return Toast.fail({
          message: `No Data，Unable to merge`, //查询条件为空
          position: "top",
        });
      await addAndRemove(this.list, {
        type: 1,
    ...
    async getList() {
      ...
      setTimeout(() => {
        ...
        if (this.searchList.length > 0) {
          ...
          this.$nextTick(() => {
            this.$refs.focusInputRef2.focus();
          });
          // 清空列表数据
          this.finished = false;
          // 重新加载数据
